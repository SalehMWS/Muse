...
func run(m *testing.M) int {
	...
	cfg, err := config.Load()
	if err != nil {
		fmt.Fprintln(os.Stderr, "postgres integration tests: load config:", err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "postgres integration tests: skipping, cannot create pool:", err)
		return m.Run()
	}
	if err := candidate.Ping(ctx); err != nil {
		fmt.Fprintln(os.Stderr, "postgres integration tests: skipping, cannot reach postgres:", err)
		candidate.Close()
		return m.Run()
	}

	if err := migrateUp(cfg, root); err != nil {
		fmt.Fprintln(os.Stderr, "postgres integration tests: run migrations:", err)
		candidate.Close()
		return 1
	}
